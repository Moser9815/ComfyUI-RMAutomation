"""
RM Video Combine - Video output node with metadata embedding.
...
logger = logging.getLogger("RMVideoCombine")
...
if 'RM_video_formats' not in folder_paths.folder_names_and_paths:
    folder_paths.folder_names_and_paths["RM_video_formats"] = ((),{".json"})
if len(folder_paths.folder_names_and_paths['RM_video_formats'][1]) == 0:
    folder_paths.folder_names_and_paths["RM_video_formats"][1].add(".json")
...
def get_video_formats():
    format_files = {}
    for format_name in folder_paths.get_filename_list("RM_video_formats"):
        format_files[format_name] = folder_paths.get_full_path("RM_video_formats", format_name)
    if os.path.exists(base_formats_dir):
...
def apply_format_widgets(format_name, kwargs):
    if os.path.exists(os.path.join(base_formats_dir, format_name + ".json")):
        ...
        video_format_path = folder_paths.get_full_path("RM_video_formats", format_name)
...
class RMVideoCombine:
    """
    RM Video Combine - Combines images into video with full format support and metadata embedding.
    """
        ...
        **kwargs
        ...
        if save_output:
            if output_dir and output_dir.strip():
                full_output_folder = output_dir.strip()
            else:
        ...
        first_image_file = f"{filename}.png"
        ...
        if format_type == "image":
            ...
            if a_waveform is not None:
                output_file_with_audio = f"{filename}-audio.{video_format['extension']}"
        ...
        if save_output and output_files:
            ...
            temp_preview_path = os.path.join(temp_dir, os.path.basename(file))
            try:
                shutil.copy2(final_file, temp_preview_path)
                logger.info(f"Copied video to temp for preview: {temp_preview_path}")
...
NODE_CLASS_MAPPINGS = {
    "RMVideoCombine": RMVideoCombine,
}

NODE_DISPLAY_NAME_MAPPINGS = {
    "RMVideoCombine": "RM Video Combine",
}
